...
const db = createClient({
  url: process.env.TURSO_DATABASE_URL || 'libsql://ai-resume-refine-khairunnisa10.aws-us-west-2.turso.io',
...
async function initializeDatabase() {
  try {
    ...
    await db.execute(`CREATE TABLE IF NOT EXISTS optimization_history (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      user_id INTEGER,
    ...
    const demoUser = await db.execute({
      ...
      args: ['demo@airesumerefine.com']
    });
    
    if (demoUser.rows.length === 0) {
      await db.execute({
        sql: `INSERT INTO users (email, name, password_hash, plan, max_optimizations, optimizations_used) 
              VALUES (?, ?, ?, ?, ?, ?)`,
        args: ['demo@airesumerefine.com', 'Demo User', 'demo_hash', 'top', 30, 0]
      });
...
app.post('/api/demo-login', async (req, res) => {
  try {
    ...
    const result = await db.execute({
      sql: `SELECT id, email, name, plan, max_optimizations, optimizations_used,
...
app.post('/api/optimize-resume', authenticateToken, async (req, res) => {
  try {
    ...
    await db.execute({
      sql: `UPDATE users SET optimizations_used = optimizations_used + 1 WHERE id = ?`,
      args: [userId]
    });
    
    await db.execute({
      sql: `INSERT INTO optimization_history (user_id, original_resume, job_description, optimized_resume, match_score) VALUES (?, ?, ?, ?, ?)`,
      args: [userId, resumeText.substring(0, 1000), jobDescription, result.optimizedResume, result.matchScore]
    });
    
    const updatedUserResult = await db.execute({
      sql: `SELECT max_optimizations, optimizations_used, (max_optimizations - optimizations_used) as remaining_optimizations FROM users WHERE id = ?`,
      args: [userId]
    });
...
app.get('/api/history', authenticateToken, async (req, res) => {
  try {
    const result = await db.execute({
      sql: `SELECT id, job_description, optimized_resume, match_score, created_at 
...
app.delete('/api/history/:id', authenticateToken, async (req, res) => {
  ...
  try {
    const checkResult = await db.execute({
      sql: `SELECT user_id FROM optimization_history WHERE id = ?`,
      args: [id]
    });
    ...
    await db.execute({
      sql: `DELETE FROM optimization_history WHERE id = ?`,
      args: [id]
    });
    res.json({ success: true });
  } catch (error) {
...
app.delete('/api/history', authenticateToken, async (req, res) => {
  try {
    await db.execute({
      sql: `DELETE FROM optimization_history WHERE user_id = ?`,
      args: [req.user.userId]
    });
    res.json({ success: true });
  } catch (error) {
...
app.post('/api/reset-password', async (req, res) => {
  ...
  try {
    await db.execute({
      sql: `UPDATE users SET password_hash = ? WHERE id = ?`,
      args: [hashedPassword, resetData.userId]
    });
  ...
  for (const user of demoUsers) {
    ...
    if (existing.rows.length === 0) {
      await db.execute({
        sql: `INSERT INTO users (email, name, password_hash, plan, max_optimizations, optimizations_used) 
              VALUES (?, ?, ?, ?, ?, ?)`,
        args: [user.email, user.name, hashedPassword, user.plan, user.max_optimizations, 0]
      });
      results.push({ ...user, status: 'created' });
    } else {
      await db.execute({
        sql: `UPDATE users SET name = ?, plan = ?, max_optimizations = ? WHERE email = ?`,
        args: [user.name, user.plan, user.max_optimizations, user.email]
      });
...
// ============= CHECK DEMO USERS =============
app.get('/api/check-demo-users', async (req, res) => {
  try {
    const result = await db.execute({
      sql: `SELECT email, plan, max_optimizations, optimizations_used FROM users WHERE email LIKE 'demo%'`
    });
    res.json({ users: result.rows });
  } catch (error) {
    console.error('Check demo users error:', error);
...
app.post('/api/test-payment-success', async (req, res) => {
  ...
  try {
    ...
    await db.execute({
      sql: `INSERT INTO users (email, name, password_hash, plan, max_optimizations, optimizations_used) 
            VALUES (?, ?, ?, ?, ?, ?)`,
      args: [email, name, hashedPassword, plan.toLowerCase(), maxOptimizations, 0]
    });
...
app.listen(PORT, () => {
  ...
  console.log(`   Turso Database: ${process.env.TURSO_DATABASE_URL ? '✅ Configured' : '❌ Missing'}`);
  console.log(`========================================`);
});
